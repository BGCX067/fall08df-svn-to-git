...
% stopband error should decrease by 20% or becomes 4/5th the original value
% which means the pasband error becomes 5/4th the original value.
% W_stop/W_pass = delta_p/delta_s
%               = 25/16
figure 
subplot(2,2,1);
stem(h_uni);
...
W_more = [ 16 25];
h_uni_wtd = firpm(N, F, A, W_more);

H_uni_wtd = fft(h_uni_wtd, N_fft);
H_uni_wtd = H_uni_wtd(1:N_fft/2);

H_uni_ang_wtd = angle(H_uni_wtd);

g_delay_uni_wtd = grpdelay(h_uni_wtd, 1 , N_fft/2);
...
hold on
stem(h_uni_wtd,'-r+');
legend('uniform','weighted');
% title('Impulse response of the filter with Hamming window');

subplot(2,2,2);
hold on
semilogy(w, abs(H_uni_wtd), '-r+', 'markersize',4)
legend('uniform','weighted');
% title('Magnitude response [Hamming]');

subplot(2,2,3);
hold on
plot(w, H_uni_ang_wtd, '-r+', 'markersize',4)
legend('uniform','weighted');
% title('Phase response [Hamming]');

subplot(2,2,4);
hold on
plot(w, g_delay_uni_wtd,'-r+', 'markersize',4);
legend('uniform','weighted');
% title('Group Delay [Hamming]')
ylim([0 20])
...
% semilogy(w, abs(H_uni));
% hold on;
% semilogy(w, abs(H_uni_wtd), '-r+', 'markersize', 2 );
% title('Comparision of filter with Uniform weights to non-uniform weights');
% legend('Uni wts','non-uni wts');
% 
% subplot(2,2,2)
% text(0,0,' Uniform window. Uniform weights vs Non-uniform weights');
% axis off
% 
% 
% subplot(2,2,3)
% zplane(h_uni, 1)
% title('Pole zero plot of the filter with uniform weights[uniform]');
% 
% subplot(2,2,4)
% zplane(h_uni_wtd, 1)
% title('Pole zero plot of the filter with non-uniform weights[non-uniform]');
...
subplot(2,2,3)
zplane(h_uni_wtd, 1)
title('uniformed window, weights band errors')
